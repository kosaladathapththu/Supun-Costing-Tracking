...
let app,auth,db,storage,analytics;if(firebaseEnabled){app=initializeApp(config);auth=getAuth(app);db=getFirestore(app);storage=getStorage(app);isSupported().then(supported=>{if(supported)analytics=getAnalytics(app)});}export{app,auth,db,storage,analytics};
